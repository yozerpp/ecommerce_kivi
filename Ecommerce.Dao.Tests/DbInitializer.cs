...
public class Tests
{
    ...
    public void Setup() {
        _dbContextOptions = new DbContextOptionsBuilder<DefaultDbContext>()
            ...
            .EnableSensitiveDataLogging(false).EnableServiceProviderCaching().ConfigureWarnings(w=>w.Ignore(CoreEventId.DetachedLazyLoadingWarning)).Options;
    }
    [Test, Order(1)]
    ...
    private const int ProductReviewCount = ProductCount * CustomerCount / 100;
    ...
    private const int CartCount = CustomerCount * 2;
    private const int CartItemCount = CartCount *10;
    private const int SessionCount = CustomerCount * 2;
    private const int OrderItemCount = CartItemCount;
    private const int OrderCount = CustomerCount * 4;
        ...
        using var initializer = new DatabaseInitializer(
           ...
           _dbContextOptions,
            new Dictionary<Type, int?> {
                ...
                {typeof(ImageProduct), ProductCount},
                {typeof(Payment), OrderCount},
                {typeof(Shipment), OrderItemCount*3/4},
